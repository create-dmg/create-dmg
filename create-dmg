...
function hdiutil_retry() {
	...
	local log_file="$(mktemp -t createdmg.log.tmp.XXXXXXXXXX)" hdiutil_retries=0 exit_code=0
	until
		(( ++hdiutil_retries ))
		...
		(( "$(grep -w -c 'Resource busy' "$log_file")" == 0 )) && exit $exit_code
	do
...
while [[ "${1:0:1}" = "-" ]]; do
	case $1 in
		...
		--skip-jenkins)
			...
			shift;;
		--applescript-sleep-duration)
			ERROR_1728_WORKAROUND_SLEEP_DURATION="$2"
			shift; shift;;
		--skip-finalize)
			SKIP_FINALIZE=1
			shift;;
